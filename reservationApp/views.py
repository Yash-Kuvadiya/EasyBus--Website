...
from EasyBus.settings import MEDIA_ROOT, MEDIA_URL
import json
...
from reservationApp.models import Booking, Category, Location, Bus, Schedule,Seats
...
def home(request):
    ...
    context['upcoming_trip'] = Schedule.objects.filter(Q(status = 1) & Q(schedule__gt = datetime.now())).count()
...
def save_schedule(request):
    resp = {'status':'failed','msg':''}
    if request.method == 'POST':
        viaList = request.POST.getlist('viaList')
        ...
        # Extract time_taken_hours and time_taken_minutes from POST data
        time_taken_hours = int(request.POST.get('time_taken_hours', 0))
        time_taken_minutes = int(request.POST.get('time_taken_minutes', 0))

        # Update or create Schedule instance with form data
        if schedule is None:
        ...
        if form.is_valid():
            ...
            schedule.time_taken_hours = time_taken_hours
            schedule.time_taken_minutes = time_taken_minutes
            schedule.save()

            # Add via locations to the schedule
            if viaList:
                for id in viaList:
                    loc_obj = Location.objects.filter(id=id).first()
                    schedule.via.add(loc_obj)

            # Create seats for the schedule
            seat_count = schedule.bus.seats
            for i in range(seat_count):
                new_seat = Seats.objects.create(seat_number=i+1)
                schedule.seats.add(new_seat)
...
#         # Added after making documentation
#         via = {}
        
#         for l in locations:
#             for v in schedule.via.all():
#                 if(l.id == v.id):
#                     via[l.id] = {"flag":True,"location":l.location}
#                     break
#             else:
#                 via[l.id] = {"flag":False,"location":l.location}
...
def manage_schedule(request, pk=None):
    ...
    if pk is not None:
        ...
        # Prepare data for via locations checkboxes
        via_location_ids = list(schedule.via.values_list('id', flat=True))
        context['via'] = via_location_ids
    else:
        context['schedule'] = {}
        context['via'] = []
...
def delete_schedule(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if not schedule_id:
                resp['msg'] = 'No schedule ID provided.'
            ...
            resp['msg'] = 'Schedule does not exist.'
        except Exception as err:
...
def scheduled_trips(request):
    if not request.method == 'POST':
        ...
        schedules = Schedule.objects.filter(Q(status = 1) & Q(schedule__gt = datetime.now())).all()
        ...
        schedules = Schedule.objects.filter(Q(status = 1) & Q(Q(Q(schedule__year = year) & Q(schedule__month = month) & Q(schedule__day = day) )) & Q(Q(depart = depart) & Q(destination = destination ))).all()
        context['schedules'] = schedules
...
def save_booking(request):
    ...
    if not request.user.is_authenticated:
        resp['msg'] = "login required"
    elif request.method == 'POST':
        
        print(request.POST.getlist('all_seats'))
        if (request.POST['id']).isnumeric():
        ...
        if form.is_valid():
            form.save()
            if booking is None:
                selected_seats = request.POST.getlist('all_seats')
                booking = Booking.objects.last()
                booking.owner = request.user
                booking.seats = len(selected_seats)
                ...
                for seat in selected_seats:
                    seat_obj = Seats.objects.filter(id=seat).first()
                    seat_obj.owner = request.user
                    seat_obj.is_booked = True
                    seat_obj.save()
                    booking.book_seats.add(seat_obj)
                    
                    # total fare calc
                    # payable = int(request.POST['payable'])
                    # booking.fare = payable
                    
                    # 
                    booking.save()
                
                
                messages.success(request, f'Booking has been saved successfully. Your Booking Reference Code is:<i> <b>{booking.code}</b> </i>', extra_tags = 'stay')
            else:
...
def myBookings(request):
    ...
    if booking.first() is None:
        context['all_bookings']  = None
    else:
        context['all_bookings'] = booking
    
    return render(request,'my_booking.html',context=context)
...
def delete_booking(request):
    if request.method == "POST":
        ...
        for seat in booking.book_seats.all():
